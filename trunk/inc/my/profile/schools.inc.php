...
$smarty->assign('Title','School course');

/*if(!$_SESSION['my_profile_personal'])
{*/
	$t_name = 'user_schools';
        $t_fields = implode(',', array_keys($table_fields[$t_name]));

        $user_schools = $db->query('SELECT ! FROM ! WHERE id=? ORDER BY year', array($t_fields, $t_name, $_SESSION['my_id']) );

	if(!DB::isError($user_schools) )
	{	
		$table_info = $user_schools->tableInfo(); 					// Recuperation des infos de la table
		$idx = 0;
		while($profile = $user_schools->FetchRow()) 						// On recupere le profil
		{
			foreach($table_info as $item)							// Pour chaque colonne de la table
			{
				...
				$key = $item['name'];			// on genere la cle qui sera utilisé dans le tableau exporté a Smarty, ca sera le label traduit a partir du nom de la colonne (trouvé dans labels.inc.php)
				$my['profile']['schools'][$idx][$key] = _($value);				// enfin, on attribue la valeur trouvé traduite...
			}
			$idx++;
		}
	}
	else
		error_log($_SERVER['HTTP_HOST'].' | '.__FILE__.' | '.$user_schools->getUserInfo());
