...
if( $_POST['login'] && $_POST['md5_passwd'])
{
	$rpc_login = new Logikse($dsn_logikse);
	$res = $rpc_login->check(stripslashes($_POST['login']), $_POST['md5_passwd']);
	if(Logikse::isError($res))
	{
		switch($res->getErrorNo())
		{
			case LOGIN_NOTSUBSCRIBED:
				$_SESSION['subscribe']['login'] = stripslashes($_POST['login']);
				$_SESSION['subscribe']['passwd'] = md5(stripslashes($_POST['md5_passwd']));
				header('location: /pub/subscribe');
				break;

			default:
				if($res->getMessage())
					$_SESSION['error']['msg'] = $res->getMessage();
				else
					$_SESSION['error']['msg'] = 'Unknow error #'.$res->getErrorNo();
				header('location: /pub');
		}
	}
	else
	{
		$user =& $db->getRow('SELECT id, login, fname, lname, nick, status FROM user WHERE login=?', array($res['login']));
		if( !DB::isError($user) )
		{
			session_destroy();
		...
		{
			$_SESSION['error']['msg'] = "Wrong login";
			header('Location: /pub'); // Wrong login / pass
		}
	}
}
else
{
	$_SESSION['error']['msg'] = "No login";
	header('Location: /pub'); // no login/pass
}
?>
