...
$smarty->assign('Title','Personal Profile');

/*if(!$_SESSION['my_profile_interests'])
{*/

	$t_name = 'user_personal';
        $t_fields = implode(',', array_keys($table_fields[$t_name]));

        $user_personal = $db->query('SELECT ! FROM ! WHERE id=?', array($t_fields, $t_name, $_SESSION['my_id']) );
	...
	{	
		...
		foreach($table_info as $item)							// Pour chaque colonne de la table
		{
			...
			$my['profile']['personal'][$key] = _($value);				// enfin, on attribue la valeur trouvé traduite...
		}
	}
	else
