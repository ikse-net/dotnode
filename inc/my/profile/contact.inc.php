...
$_SMARTY['Title'] = 'Contact Profile';
	...
	$t_name = 'user_contact';
        ...
        $user_contact = $db->query('SELECT ! FROM ! WHERE id=?', array($t_fields, $t_name, $_SESSION['my_id']) );
	...
	{	
		...
		foreach($table_info as $item)							// Pour chaque colonne de la table
		{
			...
			$my['profile']['contact'][$item['name']]['key'] = $table_fields['user_contact'][$item['name']];
			$my['profile']['contact'][$item['name']]['value'] = _($value);				// enfin, on attribue la valeur trouvé traduite...
		}
	}
	else
