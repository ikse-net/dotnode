...
$smarty->assign('Title','Interests Profile');
...
{*/
	$t_name = 'user_interests';
        ...
        $user_interests = $db->query('SELECT ! FROM ! WHERE id=?', array($t_fields, $t_name, $_SESSION['my_id']) );
	...
	{	
		...
		foreach($table_info as $item)							// Pour chaque colonne de la table
		{
			...
			$my['profile']['interests'][$key] = _($value);				// enfin, on attribue la valeur trouvé traduite...
		}
	}
	else
