...
if(is_numeric($token[2]) )
{
	...
	$friends_r = $db->query('SELECT cache_user.id, fname, lname, nb_friends FROM cache_user LEFT JOIN relation USING (id) WHERE relation.id_friend=? AND NOT FIND_IN_SET(?, cache_user.communities_id)', array($_SESSION['my_id'], $token[2]));
	...
	$_SMARTY['Title'] =  $community['info']['name'];
