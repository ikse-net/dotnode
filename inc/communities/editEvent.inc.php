...
if(is_numeric($token[2]))
{
	...
	$event['info'] = $db->getRow('SELECT id_comm,  id , title, details, country, location, city, author, date_event FROM community_event WHERE id_event=? AND id=?', array($token[2], $_SESSION['my_id']));
	if(!$event['info'])
	{
