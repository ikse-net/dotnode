...
foreach(array_keys($table_fields['blog_categorie']) as $key)
{
		...
		{
			header('Location: /error/13');
			exit();
		}
                else
                        $cat_values[$key] = stripslashes($value);

        }
}

$cat_values['id'] = $_SESSION['my_id'];

$result =& $db->autoExecute('blog_categorie', $cat_values, DB_AUTOQUERY_INSERT);
